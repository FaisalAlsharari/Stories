...
// bcryptjs docs: https://github.com/kelektiv/node.bcryptjs.js
const bcryptjs = require('bcryptjs')

// see above for explanation of "salting", 10 rounds is recommended
const bcryptjsSaltRounds = 10
  ...
  Promise.resolve(req.body.credentials)
    ...
    .then(hash => {
      ...
      return {
        ...
        img: req.body.credentials.img,
        bio: req.body.credentials.bio
      }
    })
...
router.post('/sign-in', (req, res, next) => {
  ...
  User.findOne({
    ...
    .then(record => {
      ...
      user = record
      // `bcryptjs.compare` will return true if the result of hashing `pw`
...
router.patch('/change-password', requireToken, (req, res, next) => {
  ...
  User.findById(req.user.id)
    ...
    .then(() => bcryptjs.hash(req.body.passwords.new, bcryptjsSaltRounds))
    .then(hash => {
